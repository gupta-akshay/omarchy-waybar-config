...
If you see a permission error, run `chmod +x install.sh` (or use `bash install.sh`). The script installs/updates the required AUR packages via `yay`, backs up your existing `~/.config/waybar` directory, and copies the config (`config.jsonc`, `modules/`, `style.css`, `cava.sh`, `net_speed.sh`, `waybar-gpu.sh`) into place.
...
3. Clone this repo somewhere temporary and copy the required files into your Waybar directory:
   ...
   cp -a /tmp/wbr/{config.jsonc,style.css,modules,cava.sh,net_speed.sh,waybar-gpu.sh} ~/.config/waybar/
   rm -rf /tmp/wbr
   ```
...
4. Ensure the scripts are executable:
   ...
   chmod +x ~/.config/waybar/waybar-gpu.sh
   ```
...
- `waybar-gpu.sh` – Script backing the optional `custom/gpu` module (**NVIDIA only for now**, requires `nvidia-smi`).
